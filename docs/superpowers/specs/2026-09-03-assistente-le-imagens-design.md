...
| D6 | `add_expenses` confirma? | **Sim, é `destructive: true`** — ao contrário do `add_expense` singular, que escreve já. Escrever 15 linhas sem as ver primeiro é mau negócio. O `PendingActionCard` mostra a lista toda; um Undo desfaz o lote inteiro. |
...
| D10 | `update_balance` e contas personalizadas | **Consertar.** Hoje valida o par `{account_bank, account_type}` contra `ACCT_TEMPLATES` e escreve `custom: false` fixo (`src/lib/aiTools.js:588,612`), por isso devolve `not_found` para qualquer conta criada pelo utilizador — cartões incluídos. A action por baixo (`addBalanceReading`) já suporta contas custom; a ferramenta é que é mais estreita do que ela. Passa a resolver por `listAccounts()`, como o `BalanceUpdateSheet` faz. **Os argumentos não mudam**: continuam `{account_bank, account_type}`, e a resolução procura o par em `listAccounts()` (que devolve contas de template e personalizadas) em vez de em `ACCT_TEMPLATES`; o `custom` passa a vir da conta encontrada em vez de ser `false` fixo. Se o par bater em mais do que uma conta, devolve `{error:'ambiguous_account'}` com a lista, como o `add_expense` já faz para `acct`. |
